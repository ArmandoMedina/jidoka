...
- Dado que un ítem del `ROADMAP.md` no declara su clase (sección fuera de las 4 de servicio + Referencia) o le falta `[alta:AAAA-MM-DD]` (los Urgente/Con fecha/Normal además `apetito:Nh` **o** `apetito:Nm` —horas o minutos enteros, R6—; los Con fecha además `vence:`), cuando corro `tools/verificar.ps1`, entonces el check `[contrato-roadmap]` **BLOQUEA** el push (exit 1) nombrando qué falta — la cola se clasifica, no se rankea.
- Dado que `tools/flujo.json` trae `roadmap.procedencia:true` y un ítem vivo del `ROADMAP.md` no cita su origen (ni informe `docs/analisis/`, ni récord `docs/sprints/`, ni ADR, ni `#issue`), cuando corro `tools/verificar.ps1`, entonces el check `[contrato-roadmap]` **BLOQUEA** el push (exit 1) exigiendo la procedencia — un pendiente sin origen no se puede ejecutar ni validar (ADR 0057; opt-in por instancia, aplica a toda clase viva).
- Dado que `tools/flujo.json` trae `roadmap.guion_revision:true` y un ítem **ejecutable** (Urgente/Con fecha/Normal) del `ROADMAP.md` no declara cómo se revisa (ni cita un informe `docs/analisis/` con sección «Qué debe revisar el dueño», ni un récord `docs/sprints/`), cuando corro `tools/verificar.ps1`, entonces el check `[contrato-roadmap]` **BLOQUEA** el push (exit 1) — un pendiente sin guion no es ejecutable (ADR 0057 enmienda; el icebox «Algún día» va exento).
