...
- Dado que falta `tools/blast-radius.json` (la ley), cuando corre un Stop hook (`andon`/`review`/`gemba`/`validador-stop`), entonces **falla cerrado** (exit 2, «no apruebo a ciegas») en vez de dejar cerrar en silencio; y dado que el hook `PreToolUse` (`candado-pretooluse`) **truena** (SyntaxError/crash), entonces su envoltorio a prueba de parseo **emite DENY** — el silencio deja de significar pase (ADR 0058, sprint 28).
