...
- Dado que voy a construir un sprint, cuando corro `/jidoka:planea`, entonces exige el QUÉ aprobado por el cliente (R0 con STOP) antes de la primera línea de código y la **aprobación formal del plan antes de archivarlo** (STOP 2 — un plan rechazado se ajusta y se re-presenta), y su Verificación debe poder demostrarse **sin código ni terminal** (disparo `demo-que-corre-el-cliente`); `/jidoka:cierra` no la da por cumplida si solo corre por terminal (ADR 0030).
