...
- Dado que el motor nuevo asume una pieza de instancia que el hijo no tiene (un stub del manifiesto), cuando corro `-Actualizar`, entonces se siembra en la misma pasada (`[MIGRA]`, no-clobber estricto) — y lo condicionado a arquetipo solo se decide si el sello registra `producto`; con sello viejo se avisa, no se adivina (ADR 0037, cosecha #7).
