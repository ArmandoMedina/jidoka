...
| Pieza | Qué hace |
|---|---|
| [`tools/blast-radius.json`](../tools/blast-radius.json) | La ley. 9 áreas (incluida `raiz`, la tierra de nadie; `ritual`, los comandos `/jidoka:*` y las skills; `atlas`, el aviso comando→diagrama del atlas de procesos, ADR 0033; y `kit`, el instalador y el kit sembrable — Sprint 3). Dimensiones por área: `doc_bloquea` (**un** solo real: un ADR nuevo debe listarse en su índice), `doc_avisa`, **`product_avisa`** (el grafo de `product/`), **`revisa`** (marca el código que pide `/code-review`) y `rol` (`revisor-visual` enciende `gemba-stop`; `validador` enciende `validador-stop`). Los mensajes enseñan cuándo **no** aplican (anti-fatiga). |
