...
| Pieza | Qué hace |
|---|---|
| [`tools/blast-radius.json`](../tools/blast-radius.json) | La ley. 7 áreas (incluida `raiz`, la tierra de nadie: un archivo suelto en la raíz avisa; y `ritual`, que sincroniza los comandos `/jidoka:*` y las skills-asiento con el CHANGELOG — Sprint 2); casi todo `avisa`, **un** `doc_bloquea` real (un ADR nuevo debe listarse en su índice). Los mensajes enseñan también cuándo **no** aplican (anti-fatiga de falsos positivos). |
