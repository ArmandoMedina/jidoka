...
- **`fix` — los hooks del muro fallan CERRADO (R3/R4/R5):** `review-stop` deja de dictar en su mensaje de bloqueo el comando de auto-firma y su SHA cubre ahora los archivos sin rastrear (R3); el candado `PreToolUse` deniega en duro que el agente escriba `.claude/.review-marker` / `.claude/.gemba-marker` (la firma es exclusivamente humana) y falla cerrado ante su propio crash —envoltorio + `.core` en proceso hijo— (R4); y los 4 Stop hooks salen exit 2 si falta `tools/blast-radius.json`, está vacía (`{}`), es corrupta, o carece de áreas usables (ninguna con `nombre`+`fuente`), en vez del pase silencioso (R5).
- **`fix` — el apetito expresa menos de una hora con cota (R6):** el check acepta `Nh` o `Nm` (entero `1..999h` o `1..60000m`, `>0`, nada pegado: `0h`, `30m5h`, `999999m` bloquean) y `estado-flujo` los suma correcto (`apetito:30m` → 0.5h); el backlog deja de sobreestimar el presupuesto de atención del dueño.
...
- **`fix` — las 4 curas del `/code-review` del dueño:** la rama de guion R2 ahora exige que el record `docs/sprints/` **resuelva a archivo existente** (antes solo alcanzaba la sección); `gemba-stop`/`validador-stop` ganan `-c core.quotepath=false` como `review-stop` (los nombres no-ASCII ya no se escapan → cierra un fail-open); el regex de apetito de la vista `estado-flujo` se alinea byte a byte al del gate; y la procedencia deja de aceptar un `#` incidental pegado a palabra (solo cuenta el `#issue` real).
...
- **ADR 0058** — los hooks del muro fallan cerrado: crash y ley (ausente/vacía/corrupta/sin-áreas-usables) ya no son un pase silencioso; el candado deniega escritura al marcador humano · aceptado.
...
Corte único que consolida sobre `v1.28.0` dos sprints paralelos —el guardián de ADRs y la tubería derivada del repo entero— más el fix de encoding de la foto.
...
- **`fix` — la foto de la tubería emite UTF-8 real (no CP437):** el puente Rust spawnea PowerShell sin consola y lee su stdout como UTF-8, pero PS 5.1 sin consola emite en la code page OEM **CP437** — la flecha `→` (U+2192) caía al byte `0x1A` (un carácter de control) y los acentos a bytes inválidos → el `JSON.parse` de la app rechazaba la foto (*"Bad control character"*) y dejaba **ciega** la vista "1 · La tubería". Los cinco emits a stdout que Rust consume (`tuberia-datos.ps1`, `parametrizar.ps1` ×2, `override.ps1` ×2) ahora escriben **bytes UTF-8 crudos** vía `Emit-Utf8Json`, sin pasar por `[Console]::OutputEncoding`. `probar-app.ps1` endurecido: captura bytes crudos y afirma cero caracteres de control y cero `U+FFFD` (replicando al `JSON.parse` de JS) — el test estaba **verde en falso** y ahora tiene dientes. Evidencia: `qa_runs/curas-tuberia-20260722/LOG.md`.
- **`feat` — la tubería es el mapa completo del repo (ADR 0051):** `tuberia-datos.ps1` deja de leer una **lista curada a mano** y **deriva el censo de las carpetas**: enumera `git ls-files` (tracked+untracked) y clasifica cada archivo por una tabla de convención `$TIPOS` (tipos bonitos) + un **catch-all por carpeta** (docs/sprints → "Sprints", docs/decisions → "ADRs", kanban, doctrina…) + cajón "Otros" para binarios. De **49 piezas a mano → 360 = `git ls-files` exacto**: aparecen sprints, ADRs, análisis, guías, dominios, módulos y la 5ª capacidad (CFG-1); la **Extensión VS Code** retirada **desaparece sola** (auto-cura del stale). Soltar un archivo lo muestra solo (sin editar JSON). El nombre se **deriva del propio archivo** (comandos → `/jidoka:<x>`, `.md` → su `# H1`). La UI (`app/ui/index.html`) **colapsa** las secciones grandes (caret). Aristas **vacías a propósito** — todo aparece "sin cablear" para autorarlo desde la app (sprint futuro). `tuberia-datos.ps1` re-grabado UTF-8 **con BOM** (los nombres de tipo con acento son literales). `probar-app.ps1` **41/41** (afirma `piezas ≈ git ls-files`). Récord: [`sprint-24-tuberia-por-carpetas-plan.md`](docs/sprints/sprint-24-tuberia-por-carpetas-plan.md), evidencia [`qa_runs/tuberia-carpetas-20260722/LOG.md`](qa_runs/tuberia-carpetas-20260722/LOG.md).
- **ADR 0051** — la tubería es el mapa completo del repo, derivado por convención (no una lista a mano) · aceptado.

## [1.28.0] — 2026-07-21

### El pilar de flujo, movimiento 1 — los documentos dejan de crecer (FLU-1, ADR 0049)

La Casa del TPS tiene dos pilares; el método construyó Jidoka (parar ante el defecto) y el pilar de flujo (JIT) no existía. Nace del diagnóstico medido sobre 274 MB de transcripciones y de un benchmark de 4 frentes (`docs/analisis/gemba-gestion-del-flujo-202607.md`, `docs/analisis/benchmark-flujo-202607.md`): el HANDOFF declaraba «se limpia al abrir» en su primera línea y llegó a 419 líneas con 12 secciones históricas sin que nada lo detectara. La regla del sprint: **todo tiene que ser código que rechaza la acción** — Kanban se apoya en presión social que con agentes no existe. Plan-contrato: `docs/sprints/sprint-22-pilar-de-flujo-plan.md` (aprobado 2026-07-21; apetito: 6 horas de revisión del cliente con muerte por defecto).
...
- **`feat` — el límite WIP como muro de entrada (R5, *drum-buffer-rope*):** el ritmo lo marca la **aceptación del cliente, no la producción**. `tools/estado-flujo.ps1 -Gate` (nuevo, mecánica) + clave `estado` (dato **vivo** de instancia) en `tools/flujo.json`: si hay un Gemba en `estado.gembas_pendientes` con `aceptado != true`, `/jidoka:planea` **se planta antes de R0** nombrando cuál (`[BLOQUEA] … <id> …` + `ABRIR SPRINT NUEVO BLOQUEADO`, exit 1). La aceptación se **registra en `/jidoka:gemba`** (booleano `aceptado: true` + `aceptado_fecha`, lo que desbloquea; un rechazo se queda pendiente y el rework hereda el mismo `id`) y el **cierre** (`/jidoka:cierra`) registra el Gemba nuevo (`aceptado: false`) — un sprint sin Gemba registrado es uno que se auto-declaró aceptado, y eso no existe. Es **gate de comando, no de push** (el que bloquea pushes sigue siendo `verificar`). Entrada sin `id` → AVISO + cuenta como pendiente (fail-safe); sin la clave `estado` → «no aplica» (exit 0); corrupto → falla cerrado (exit 2). Cubierto por `tools/probar-flujo.ps1` (casos w1–w6).
- **`feat` — la vista de «qué sigue» (R6, *gestión visual: el estado se ve sin preguntar*):** `tools/estado-flujo.ps1` gana **dos modos** junto al `-Gate` (intacto). **`-Json`** emite a stdout SOLO el contrato JSON —`version`, `sprint_activo`, `wip_limite`, `gembas_pendientes`, los **3 `siguientes`** del ROADMAP (Urgente por alta asc, Con fecha por vence asc, Normal por alta asc), `esperando_terceros` (todos los ítems vivos con `espera:<quien>`, agrupables), `muertos_recientes` (última entrada de `docs/MUERTOS.md`) y `conteos`— parseando el ROADMAP con las convenciones de `[contrato-roadmap]` (título = negritas `**…**`). Es **el contrato que la cara Tauri pintará después**: cero UI aquí. El **modo default es el resumen humano** (<15 líneas). Y el **primer hook `SessionStart` del repo** (`.claude/hooks/flujo-sessionstart.ps1`, cableado en `.claude/settings.json`, hoy sin ninguno) **inyecta ese resumen al abrir sesión** — el estado se empuja a la vista de forma determinista, ya no depende de que un comando se acuerde; resuelve el «no tengo a la vista lo que hay que hacer» medido en el diagnóstico. Sin `tools/flujo.json`/ROADMAP → JSON mínimo válido (degrada, no truena); corrupto → falla cerrado (exit 2); el hook sin `tools/estado-flujo.ps1` sale 0 silencioso. Cubierto por `tools/probar-flujo.ps1` (casos j1–j4) y la prueba de vida del hook en `tools/probar-hooks.ps1`.
- **`feat` — el reporte de avance para terceros (R7, *gestión visual para quien no está en la terminal*):** `tools/reporte-avance.ps1` (nuevo, mecánica) arma un **`.html` autocontenido** (cero deps/CDN/servidor, se abre con doble clic; UTF-8 sin BOM; SVG inline estático; claro/oscuro; legible en móvil) con **5 secciones en lenguaje LLANO** derivadas del estado real: **qué se terminó** (del CHANGELOG tope), **en qué vamos** (un **hill chart de Basecamp** — subir la loma = resolver incógnitas, bajar = ejecutar lo conocido; **sin porcentajes**; los movimientos y su posición salen del HANDOFF), **qué espera respuesta**, **qué se descartó** (última entrada de `docs/MUERTOS.md`, o «Nada se ha descartado») y **qué sigue** (los 3 siguientes con su tamaño en llano), reusando `estado-flujo.ps1 -Json`. Toda la **jerga se traduce** (`gate`/`WIP`/`commit`/`PR`/`rebanada`/`sprint` → «control automático»/«límite de trabajo abierto»/«registro»/«revisión»/«entrega parcial»/«ciclo de trabajo»): el cliente se lo manda **tal cual** a un tercero no-técnico (la autoridad del dominio de su lab, o su socio) y lo entiende en <5 min. **Es VISTA, NO gate** (no bloquea nada, ADR 0002) y **degrada con gracia** si falta una fuente; nadie lo redacta a mano. Resuelve el «**no tengo sensación de avance ni para mí ni para Marcelo**» del diagnóstico. Cubierto por `tools/probar-flujo.ps1` (casos r1–r3).
- **`feat` — el reparto de funciones en casa propia (R8a, *quién hace qué y qué NO le toca*):** nace del pedido del cliente (*«qué función voy a tomar yo… para saber qué hace cada quien»*) y su duda (*«el usuario debería ser siempre el que sabe… pero en enti no sé cómo ponerme»*). El casting sale de `product/infra.md` a `product/casting.md` (nuevo, gobernado capa-2) con molde sembrable `kit/.jidoka/templates/casting.md`; la sección `## El casting` de la plantilla infra.md queda como **puntero de una línea** (se conserva el encabezado para no romper el gobierno de secciones sobre los infra viejos). El diseño parte el humano en **dos roles**: la **autoridad del dominio** («el que sabe», cuya ventana de atención es la restricción de Goldratt, no opera la IA) y el **dueño-operador** (prioriza, corre el método, acepta el flujo). En la nave ambos = el cliente; la respuesta a la duda: en un dominio ajeno la autoridad es otra persona y ser solo dueño-operador **es correcto, no un hueco**. `docs-gobernados.json` gana la entrada capa-2 (`estado-docs.ps1` → `[CONFORME]`); `/jidoka:arranca` §2 inyecta `@product/casting.md` con su preflight. Análisis solo-lectura del caso enti en `docs/analisis/reparto-enti-202607.md` (Marcelo = autoridad, ventanas async ~1 semana; el cliente = dueño-operador). El detalle conductual por asiento llega en R8b.
- **`feat` — el casting con enfoque real (R8b, *cada asiento piensa distinto*):** los 4 agentes-asiento (`.claude/agents/{explorador,mecanico,auditor,arquitecto}.md`) dejan de ser el mismo prompt genérico — cada uno gana **cómo piensa** y **qué reporta por su cuenta**, conservando su alcance negativo. Cada cuerpo suma `## Cómo piensas` (el sesgo de oficio) y un `## Tu reporte` de estructura fija: **explorador → cobertura** (¿dónde más vive esto?; convenciones vistas, dirección no barrida, contador N archivos/M matches), **mecánico → fidelidad** (el spec es contrato; si difiere, se detiene, no improvisa), **auditor → refutar** (busca el caso que rompe; distingue *«verifiqué X»* de *«no encontré evidencia contra X»*), **arquitecto → caminos** (≥2 opciones con costo/riesgo, qué rompería cada una y cuál es reversible; recomienda por criterio, no por gusto). Todo reporte trae la sección **obligatoria «Lo que noté por mi cuenta»** —el hallazgo no pedido— aunque diga «nada». El lint `tools/probar-agentes.ps1` **lo hace cumplir** (exige `## Cómo piensas`, `## Tu reporte` y la frase, con el frontmatter íntegro). Misma pieza en todos los proyectos: baja por el manifiesto. `product/casting.md` guarda la carta de una línea; el detalle conductual vive en cada agente.
- **`feat` — el pilar viaja al kit:** `estado-flujo`/`expirar`/`reporte-avance`/`probar-flujo`/hook `SessionStart` al manifiesto como mecánica; `tools/flujo.json` como **stub de instancia no-clobber** (contratos default + estado vacío — el del hijo jamás se pisa en `-Actualizar`). Mordida cazada al sembrar: los stubs viejos de HANDOFF/ROADMAP/CHANGELOG **violaban los contratos nuevos** — un hijo recién sembrado nacía con el push bloqueado; reescritos a formato bajo contrato (`probar-sembrar` 38/38, `probar-instalador` 67/67). Capacidad [[FLU-1-pilar-de-flujo]] al grafo con sus 5 Gherkin (`auditar` íntegro).
...
Nace de la lección más cara del sprint anterior: construir la superficie *antes* de validarla con el cliente. En el sprint "sistema configurable" el motor quedó bien (bandeja, estatuto del ritual, candado IA) pero la cara fragmentada en comandos de VS Code lo rechazó al verlo — lo contrario de lo que había aprobado en 6 Gembas: **la maqueta como UNA app navegable**. Esta versión aplica la lección al pie de la letra: primero se valida la cara (Gemba de fidelidad **antes** de cablear un solo dato, con STOP en el plan), luego se cablea el motor. La app Tauri es la superficie que el cliente vio, aprobó y pidió — no una interpretación. El instalador NSIS es el asset del release; el `.exe` no se versiona.

- **`feat` — la app de escritorio Tauri v2 `app/` (R2, Jidoka-only, no se siembra):** cascarón byte-fiel a la maqueta congelada `docs/analisis/maqueta-tuberia-202607.html` (SHA256 `AA2F6268...` verificado). Una sola ventana 1280x800, sin servidor, sin Chromium separado. El Gemba de fidelidad se corrió con el `.exe` de debug **antes** de cablear datos — el cliente aprobó con sus ojos ("Sí es fiel… abre y se ve como me gustó"). Esa es la lección del sprint anterior aplicada: el criterio de "hecho" de R2 era el Gemba, no el `.exe`.
- **`feat` — el contrato de datos app↔motor `tools/tuberia-datos.ps1` + `tools/tuberia-piezas.json` (R3-motor):** la semilla curada de **49 piezas** y **57 aristas** extraída VERBATIM del bloque de datos de la maqueta (el plan estimó "37+42"; la semilla fiel al artefacto tiene 49/57). El consolidador invoca `bandeja.ps1 -Json` y `estado-ritual.ps1 -Json` en proceso aparte (no dot-source — el `exit 0` al final los mataría) y superpone el estado VIVO sobre cada pieza (régimen de `contratos.json`, candado, firma). Los switches `-Json` aditivos de `bandeja.ps1` y `estado-ritual.ps1` son puros — sin `-Json` la salida de consola es byte-idéntica a la de antes.
- **`feat` — la tubería con datos reales `app/ui/index.html` (R3-UI + R4-UI + R5-UI):** carga dinámica via `invoke('cargar_datos')` — al abrir, el motor PS real pinta las 49 piezas con su régimen/candado/firma reales y la bandeja con sus 34 pendientes. El formulario de alta (`altaResumen`) escribe de verdad via `invoke('parametrizar')`; el modo avanzado (`reclasResumen`) reclasifica/pone candado via `invoke('override_accion')` y muestra la **firma real derivada de `git config`** — no la teclea, no la inventa (ADR 0047). El wizard `'doc'` y el flujo `'rec'` quedan teatro y lo confiesan (su `.carton` lo dice y los redirige al formulario real).
- **`feat` — el escritor único `tools/parametrizar.ps1` (R4-motor):** port fiel de `extension/contratos.js` + `extension/ritual.js`. Upsert de contrato, agregar a la fuente de la ley, insertar `@` bajo el marcador — "acumula avisos, jamás éxito falso". Idempotente por token con borde (byte-fiel a `ritual.js`). La trampa PS 5.1 del desenvuelto de array de 1 elemento cazada y documentada. `tools/probar-parametrizar.ps1` **27/27**.
- **`feat` — la firma que no se inventa `tools/override.ps1` (R5-motor):** port fiel de `extension/contratos.js:registrarOverride+firmaDeterminista` (ADR 0047). Deriva la firma de `git config user.name/user.email` dentro del script; **aborta con error si `user.name` está vacío** — el llamador (la app) no puede pasar el firmante. El `candado-on` sobre un contrato con `regimen`+`comandos` previos los deja intactos (merge que preserva campos). La contraseña del modo avanzado pasó de `'GARANTIA-NULA'` fija al **nombre del repo** (patrón GitHub), con tolerancia a acentos. `tools/probar-override.ps1` **26/26**.
- **`refactor` (breaking) — retiro completo de la extensión VS Code `extension/` (R6):** `extension/`, `tools/probar-extension.ps1` y el área `extension` de la ley retirados (`git rm`). Los ports de `contratos.js` y `ritual.js` ya viven en `parametrizar.ps1` y `override.ps1` con sus tests completos. El gate `no-borres-el-motor` lo aceptó con el ADR 0048 en el rango. Confesión honesta: `ligas.js` no tiene port (la autoría de ligas queda manual o es capacidad futura de la app; el evaluador `estado-ligas.ps1` sigue vivo). Referencias en `tools/parametrizar.ps1` y `override.ps1` son provenance del port, no afirmaciones de superficie.
- **`build` — instalador NSIS (R7, asset del release, no versionado en git):** `npx tauri build` con `bundle.targets:["nsis"]` y `version:1.27.0` en `tauri.conf.json`. Ruta: `app/src-tauri/target/release/bundle/nsis/`. El instalador es el artefacto del release; el árbol `target/` está en `.gitignore` — el working tree queda limpio. Riesgo declarado: el AV (Bitdefender) puede llevarse el instalador.

**Migraciones (breaking):** la extensión VS Code de Jidoka deja de existir — `Ctrl+Shift+P > Jidoka` ya no muestra comandos. La superficie es la app de la tubería. Si usabas `jidoka.parametrizar` o `jidoka.reclasificar`, ahora es la app. `ligas.js` (autoría de ligas código↔capacidad) sin port aún — editar `tools/ligas.json` manualmente o esperar la capacidad futura de la app.

## [1.26.0] — 2026-07-21

### El sistema configurable, fase 1 — el motor del gobierno configurable (ADRs 0045-0047)

Del descubrimiento 2026-07-20/21 (informe + maqueta clickeable validada en **6 Gembas vivos** del cliente): Jidoka deja de ser *solo* una metodología y se vuelve un **sistema de gobierno configurable** — el usuario parametriza qué se vigila, qué se lee, qué régimen tiene cada pieza y qué no toca la IA, sin editar JSON a mano. La línea doctrinal **no se dobla**: la UI autora, el gate ejecuta (ADRs 0002/0044 intactos). Esta versión entrega el **motor** de esa visión; la superficie de la extensión (R4/R6) queda supersedida por el ADR 0048 (ver la nota honesta al final).

- **`docs` — los 3 ADRs de la identidad + CFG-1 (R1):** [ADR 0045](docs/decisions/0045-identidad-sistema-gobierno-configurable.md) (metodología → sistema de gobierno configurable), [ADR 0046](docs/decisions/0046-contratos-y-regimenes.md) (tres regímenes por pieza — `motor`/`estatuto`/`libre`; el régimen efectivo = fábrica + overrides firmados en `tools/contratos.json`, INSTANCIA no-clobber que el lazo jamás pisa) y [ADR 0047](docs/decisions/0047-meta-gobierno-contrasena-firma-candado.md) (meta-gobierno: contraseña-ritual + firma + candado IA). Nace la capacidad ancla **[[CFG-1-gobierno-configurable]]** bajo `MOD-andon`. **Rename estatuto:** por orden del cliente se retiró la analogía con una marca de tercero (un ERP) — barrido de 17 puntos + 2 valores de régimen en todo el repo, con ADR 0042 enmendado (nota honesta, no reescritura muda). `auditar` 0, `verificar` 0.
- **`feat` — la bandeja "pendiente de parametrizar" `tools/bandeja.ps1` (R2, mecánica, sembrada):** vista de solo lectura que reúne en **UNA cola** lo que ninguna regla gobierna — tres motivos (**huérfano** · **solo existe** · **desviado** capa-2). Calca los matchers byte-fieles de `estado-gobierno.ps1`/`estado-docs.ps1`; resta lo firmado en `contratos.json`; exit 0 siempre, exit 2 si no-git; `-Salida` HTML sin BOM. Sobre el repo real aflora el caso `docs/analisis/costo-neto-sgi-202607.md` y los árboles sin regla, como pedía el descubrimiento. `tools/probar-bandeja.ps1` **15/15**.
- **`feat` — el estatuto del ritual `tools/estado-ritual.ps1` (R3, mecánica, sembrada):** hermano de `estado-docs`, generaliza el estatuto (ADR 0046) del documento **al ritual**: el ledger `tools/ritual-gobernado.json` declara los `@`-includes de fábrica por comando; agregar un `@` legal ya **no** se acusa como mutilación (aditiva = OK), pero faltar un `@` de fábrica = **DESVIADO** nombrándolo. `-Estricto` = muro opt-in en CI; surtido en `arranca.md` §1b. `tools/probar-ritual.ps1` **13/13**.
- **`feat` — el candado IA `.claude/hooks/candado-pretooluse.ps1` (R5, mecánica):** el hook `PreToolUse` que **deniega en el momento** (`permissionDecision=deny`) que la IA edite (Write/Edit/Bash) una pieza con `candado:true` en `contratos.json`, nombrando el contrato y el camino legal. Calca `no-memorias-pretooluse.ps1` (falla-**abierta** si no parsea o falta el ledger — viaja a hijos sin `contratos.json`); es el **único muro determinista** del meta-gobierno (el `permissions.deny` es capa estática por prefijo, no por ruta). El disparo **`deny-vs-ask`** pasa de catálogo-solo a **cableado**. `tools/probar-hooks.ps1` **42/42**.
- **`feat` — el formulario y el modo avanzado en la extensión (R4/R6):** webview-formulario **fiel a la maqueta** (`jidoka.parametrizar` — régimen/cajón/fuerza/comandos → escribe contrato + regla + `@`) y `jidoka.reclasificar` (modo avanzado: acción → motivo obligatorio → contraseña-ritual → **firma derivada de `git config`**, aborta sin `user.name` — no la inventa, ADR 0047). Ports JS puros con encoding cross-stack sin BOM. `tools/probar-extension.ps1` **26/26** (`node --test` 28 casos).
- **Nota honesta (superficie supersedida):** la superficie de la extensión (R4/R6) queda **supersedida por el ADR 0048** — la superficie del gobierno pasa a ser **la app de la tubería (Tauri)**, fiel a la maqueta navegable que el cliente validó, porque el producto fragmentado en comandos de VS Code lo rechazó al verlo. La extensión **se retira en `v1.27.0`**. El **motor de esta versión (R1/R2/R3/R5) es la base de la app**: la app lo invoca, no lo reimplementa.
...
Sprint nacido de que el cliente sintió que `10-arranca` "era diferente" — y tenía razón. Se auditaron los 24 diagramas AS-IS contra su **fuente real** (comando `.md` / script `.ps1`), con cada hallazgo verificado y citado: **14 fieles, 10 desviados**. Plan-contrato: `docs/sprints/sprint-15-atlas-fiel-plan.md`; informe durable: `docs/analisis/fidelidad-atlas-202607.md`.
...
Seis issues llegados en batch de la primera bajada real 1.13→1.16 en un hijo instalado (caso 1). Plan-contrato: `docs/sprints/sprint-14-cosecha-7-plan.md`.
