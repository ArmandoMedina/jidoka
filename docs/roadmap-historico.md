# Roadmap histórico — el archivo muerto del roadmap-diario

> **Este archivo NO es una cola de trabajo** (a diferencia de `ROADMAP.md`, que ahora es una cola de trabajo clasificada bajo contrato — ver `tools/flujo.json` y el check `[contrato-roadmap]` de `tools/verificar.ps1`). Es el roadmap-diario anterior, guardado ÍNTEGRO tal como estaba justo antes de la migración al contrato del ROADMAP. Se consulta solo cuando se busca arqueología; la cola vigente vive en `ROADMAP.md`, lo cumplido en `CHANGELOG.md`, el porqué de las decisiones en `docs/decisions/`.

---

## Archivado 2026-07-21 (migración inicial del contrato del ROADMAP, sprint FLU-1)
