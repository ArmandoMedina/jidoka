...
# FALLA CERRADA (R5): sin la ley el muro no puede saber que areas son visuales -> NO apruebo a ciegas.
# Antes salia exit 0 (silencio, dejaba cerrar). Alineado con el criterio de fallar-cerrado del gate.
if (-not (Test-Path $manifestPath)) {
  [Console]::Error.WriteLine("BLOQUEO (gemba-stop): no encuentro la ley tools/blast-radius.json. No apruebo a ciegas: sin la ley el muro no sabe que areas exigen evidencia visual. Restaura tools/blast-radius.json (o corre el instalador) antes de cerrar.")
  exit 2
}
...
try { $manifest = Get-Content $manifestPath -Raw -ErrorAction Stop | ConvertFrom-Json -ErrorAction Stop }
  ...
  [Console]::Error.WriteLine("BLOQUEO (gemba-stop): la ley tools/blast-radius.json existe pero no puedo medirla (JSON corrupto/truncado): $($_.Exception.Message). No apruebo a ciegas: sin poder leer la ley el muro no sabe que areas exigen evidencia visual. Repara tools/blast-radius.json (o corre el instalador) antes de cerrar.")
  exit 2
}
if (-not $manifest) {
