...
3. **R5 — los 4 Stop hooks dejaban cerrar sin la ley.** `andon-stop`, `review-stop`, `gemba-stop`,
   ...
   (`exit 2`, "no apruebo a ciegas") en los cuatro.
...
|---|---|---|---|
| 1 | Self-test de hooks completo | `probar-hooks.ps1` | **60/60 [PASA]**, exit 0 (era 47/47; +13 casos R3/R4/R5) |
...
- `tools/probar-hooks.ps1` — 60/60, exit 0 (salida citada en el reporte de la tarea y en los mensajes de commit del sprint).
- Archivos tocados: `.claude/hooks/review-stop.ps1` (R3), `.claude/hooks/candado-pretooluse.ps1`
  (R4, ahora envoltorio) + `.claude/hooks/candado-pretooluse.core.ps1` (R4, nuevo — la logica real),
  `.claude/hooks/andon-stop.ps1`, `.claude/hooks/gemba-stop.ps1`, `.claude/hooks/validador-stop.ps1` (R5).
- Salidas en vivo de las tres demos A/B (R4 deny, R5 exit 2, R3-def2 diff=0) reproducibles con los comandos de arriba.

## Veredicto

Los tres defectos medidos en vivo quedan curados con rojo→verde y regresion en el self-test: el
...
aprueba a ciegas sin la ley (R5). `probar-hooks.ps1` corre **60/60 verde, exit 0**. El checkpoint
