...
**Segundo lote en el mismo PR (2026-07-16, ADR 0033):** el atlas se acopla al flujo con acoplamiento **asimétrico** (proceso=fuente, prosa+diagrama=vistas, código=implementación). Área `atlas` en el manifiesto: tocar un comando `/jidoka:*` **avisa** revisar su diagrama (no bloquea; el bloqueo se gana por regla 2-3). `probar-gate` 10/10, `verificar` confirma que el aviso `[atlas]` dispara. `andon/README.md` a 9 áreas. `10-arranca` homologado a WYSIWYG (subprocesos expandidos, sin drill-down). `atlas:sellar` sella la plataforma Camunda 7 en los `.bpmn` para que el editor visual (Miragon) abra sin preguntar; `jidoka.code-workspace` abre el repo listo. **Pendiente (humano):** revisar y mergear el PR #85; re-modelar los otros 18 diagramas de detalle por tandas (siguen happy-path).
