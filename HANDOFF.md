...
- **Sprint 3 · Fases 3.B/3.C — lo diferido (documentado en ROADMAP, nota del cliente "que no se olvide"):** los otros 2 arquetipos + la matriz ejecutable, los 12 templates de producto + PRODUCT_BRIEF + benchmark, multiplataforma (`.sh`/`pwsh`), el CLI npm + SSOT de versión + release-CI + ensayo del empaquetado, barreras code-first, y el dogfood completo del ADR 0003 (motor solo en `kit/`, Jidoka se auto-instala). Las VMs del equipo son el ambiente ideal para el smoke del instalador.
