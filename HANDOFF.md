...
- **DEUDA DE MÉTODO (la más importante, ADR 0009 → "Qué NO resuelve"):** ni `docs-as-code` ni `code-first` tienen aún un consumidor real (los labs no instalan Jidoka: son sus padres). **La validación de verdad es USAR el instalador en un repo vivo — el siguiente paso de mayor valor, por encima de más construcción.** La regla 2–3 dice: construir más arquetipos/piezas *cuando el uso lo pida*, no antes.
