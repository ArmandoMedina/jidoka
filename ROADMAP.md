...
- **Documentos gobernados: la lógica del ritual no debe desviarse en los hijos (2026-07-17, sin triar).** Observado en un lab de rescate (caso reconstrucción): aunque se corra `-Actualizar`, los `.md` del ritual (`arranca`/`planea`/`cierra`) del hijo **divergen** del canónico y se quedan sin las curas del motor (p.ej. el preflight de #104). Causa raíz: esos comandos caen en un **limbo** entre el *motor* (baja y pisa) y la *instancia* (se preserva porque es customizable) — llevan dentro **lógica gobernada** (orden de inyección, STOPs, preflight) **y** "sabor" del hijo (sus `@`, su casting). El QUÉ está diagnosticado; el fork de diseño es el mecanismo: **forzar** (los `.md` pasan a motor y pisan) · **partir** (lógica gobernada inyectada + cáscara editable fina) · **marcar+avisar fuerte** ("no se pueda **o no se recomiende** desviar" — palabras del cliente). Decisión de arquitectura con trade-offs → candidata a `/jidoka:planea` tras medir el drift real. **Nota de coordinación:** en el lab de rescate trabaja otro agente; ahí se entra **solo-lectura** o con **rama propia**.
