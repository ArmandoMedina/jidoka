...
- **Método — claridad del límite orquestador↔subagente** (lección de campo, 2026-07-11): el cliente señaló en sesión que *no tiene claro cómo funciona hoy la delegación a subagentes porque el método creció rápido*. La regla existe (`kanban/roles.md`: "el orquestador no pica código en el hilo principal"; anuncio `🎭 Asiento: <rol>`) pero no queda obvia ni cuándo aplica (lectura voluminosa y trabajo aislado → subagente; código acoplado con TDD → en sesión, anunciado). Candidato: una sección corta y operativa en `kanban/roles.md`/`lazo.md` que dé el criterio ("qué va a subagente vs qué se queda") con ejemplos, y vuelva prominente el anuncio de asiento. Es exactamente el tipo de lección que el lazo (ADR 0012) debe capturar — aplicada al propio método.
