...
- **CLI npm `npx jidoka-method init`** (distribución cross-platform) + **SSOT de versión** + **CI de release** + **smoke del instalador en CI** (lección: *un workflow que solo corre al cortar release se pudre en silencio* → `workflow_dispatch` de rescate) + **ensayo del empaquetado** (el build se autoverifica contra el manifiesto que el runtime usa).
  - ✅ **SSOT de versión + release derivado HECHO (`v1.3.0`, ADR 0020):** `tools/publicar.ps1` deriva tag+notas del SSOT (`version.txt`+CHANGELOG) y corre la suite antes de publicar (Jidoka-only). El `package.json` como SSOT se difiere hasta que exista el CLI (derivará de `version.txt`, no al revés).
  - ⏳ **Pendiente (bloqueado por verificación):** el CLI npm y la multiplataforma no se pueden *declarar* desde un entorno Windows-only sin afirmar cross-platform sin evidencia (`evidencia-no-palabra`). Necesitan un entorno no-Windows para probarse, o una decisión explícita de shippear sin probar. El `npm publish` además necesita la cuenta del cliente.
...
- **Drift estructural núcleo↔labs.** ⏳ **MEDIO RESUELTO.** (a) La clasificación genérico-vs-customizado se cerró con el fix EOL (ADR 0021) + re-sellado limpio. (b) El re-agregado recurrente (los labs rehacían los mismos back-outs cada bajada) se cerró con la **lista de exclusión del hijo** (`v1.5.0`, ADR 0022): el sello declara `excluir` y el lazo lo respeta. **Falta**: que los labs añadan su `excluir` una vez (SGI/TF), y decidir si el namespacing de comandos / skills genéricos del núcleo debe converger a la estructura plana+persona de los labs (o al revés) — esa es la decisión de estructura canónica, aún abierta.
